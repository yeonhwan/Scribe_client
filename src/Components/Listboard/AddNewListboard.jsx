...
export default function AddNewListboard({setIsAddMode, refetch}) {
  ...
  return (
    <div onClick={(e) => {clickHandler(e)}} className="flex justify-center items-center absolute top-0 left-0 w-screen h-screen bg-neutral-800/80 backdrop-blur-md">
      <div onClick={(e) => {e.stopPropagation()}} className="flex flex-col justify-center items-center w-[400px] h-[400px] bg-neutral-300 rounded-xl backdrop-blur-lg">
        ...
        <div className="flex justify-evenly">
          <button onClick={submitHandler} className="px-11 bg-slate-800 hover:bg-sky-600">save</button>
          <button className="px-10 bg-amber-900 hover:bg-amber-700 ml-2"onClick={() =>{setIsAddMode(false)}}>cancel</button>
        </div>
      </div>
    </div>
  )
